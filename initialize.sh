...
if [ ! -f src/powered/keys.py ]; then
  echo "SECRET_KEY = '"$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)"'" > src/powered/keys.py
  echo $(pwd)"/src/powered/keys.py generated"
fi
